...
export class DrawStampUtils {
    ...
    addManualAgingEffect(x: number, y: number, intensityFactor: number) {
        const brushRadius = (0.8 + intensityFactor * 3.2) * this.mmToPixel;
        const dabCount = Math.max(6, Math.round(8 + intensityFactor * 28));
        ...
        for (let i = 0; i < dabCount; i++) {
            ...
            const intensity = Math.max(0.05, intensityFactor);
            // 将点击的地方增加一个做旧数据到做旧的列表里面
            this.drawStampConfigs.agingEffect.agingEffectParams.push({
                x: adjustedX + Math.cos(angle) * distance,
                y: adjustedY + Math.sin(angle) * distance,
                noiseSize: Math.random() * (2.5 + intensity * 3) + 1,
                noise: Math.random() * 200 * intensity,
                strongNoiseSize: Math.random() * (3 + intensity * 4) + 2,
                strongNoise: Math.random() * 250 * intensity + 5,
                fade: Math.random() * 85 * intensity,
                seed: Math.random(),
                manual: true
            })
        }
    ...
    private drawPointerGuides() {
        if (!this.pointerPosition) return;
        const {x, y} = this.pointerPosition;
        const rulerWidth = RULER_WIDTH * this.mmToPixel;
        const rulerHeight = RULER_HEIGHT * this.mmToPixel;
        const mmX = Math.round(((x - rulerWidth) / this.mmToPixel) * 10) / 10;
        const mmY = Math.round(((y - rulerHeight) / this.mmToPixel) * 10) / 10;

        if (this.drawStampConfigs.ruler.showCrossLine) {
            this.drawRulerUtils.drawPositionCrossLines(this.offscreenCanvas, this.canvas, rulerWidth, rulerHeight, x, y, this.drawStampConfigs.primaryColor)
        }
        if (this.drawStampConfigs.ruler.showCurrentPositionText) {
            this.drawRulerUtils.drawCurrentPositionText(this.canvasCtx, mmX, mmY, this.scale, rulerWidth, rulerHeight)
        }
    }
    ...
    private addAgingEffect(
        ...
        const savedParams = this.drawStampConfigs.agingEffect.agingEffectParams
        const hasManualParams = savedParams.some((param) => param.manual)
        if (!this.drawStampConfigs.agingEffect.applyAging && !hasManualParams) return;
        ...
        const manualParams = this.drawStampConfigs.agingEffect.agingEffectParams.filter((param) => param.manual)
        const automaticParams = this.drawStampConfigs.agingEffect.agingEffectParams.filter((param) => !param.manual)

        // 如果需要刷新或者参数数组为空,则重新生成参数
        if (this.drawStampConfigs.agingEffect.applyAging && (forceRefresh || automaticParams.length === 0)) {
            this.drawStampConfigs.agingEffect.agingEffectParams = manualParams
        ...
        } else if (!this.drawStampConfigs.agingEffect.applyAging) {
            this.drawStampConfigs.agingEffect.agingEffectParams = manualParams
        }
     ...
     */
    saveStampAsPNG() {
        const previousRuler = {...this.drawStampConfigs.ruler}
        // 图像与边框的间距
        let imagePadding = 1
...
+            image	绘制到画板的图像资源，可以是任何的 canvas 图像源 ( CanvasImageSource)，例如：HTMLImageElement，HTMLVideoElement，或者 HTMLCanvasElement
            ...
            // 如果启用了做旧效果或存在手动画笔痕迹，在新的 canvas 上应用做旧效果
            if (this.shouldApplyAgingEffect()) {
    ...
    private shouldApplyAgingEffect() {
        return (
            this.drawStampConfigs.agingEffect.applyAging ||
            this.drawStampConfigs.agingEffect.agingEffectParams.some((param) => param.manual)
        )
    }
