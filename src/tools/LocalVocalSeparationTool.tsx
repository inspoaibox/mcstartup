...
export default function LocalVocalSeparationTool() {
    ...
    async (dir = modelDir) => {
      ...
      try {
        const [runtimeStatus, modelStatus] = await Promise.all([
          ...
          invoke<SherpaModelStatus>('sherpa_audio_check_model', { modelDir: dir, tool: 'separation' }),
        ]);
    ...
    try {
      ...
      await downloadSherpaItems(
        ...
        [
          ...
          { ...SEPARATION_MODEL_ITEM, isReady: separationReady },
        ],
        (percent, label) => {
  ...
  async function separate() {
    ...
    setError('');
    try {
      const response = await invoke<SeparationResult>('sherpa_audio_separate', {
        modelDir,
        inputPath,
  ...
  return (
    <div className="flex h-screen flex-col bg-gray-50 text-gray-800 dark:bg-gray-900 dark:text-gray-100">
      <ToolHeader
        icon="🎚️"
        title="本地人声 / 伴奏分离"
        subtitle="sherpa-onnx + Spleeter 2-stem，本地离线分离"
      />
        ...
        <aside className="flex min-h-0 flex-col gap-4 overflow-y-auto pb-4 pr-1">
          <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="mt-3 rounded-lg bg-gray-50 p-3 text-xs leading-5 text-gray-500 dark:bg-gray-900/50 dark:text-gray-400">
              ...
              <div>Spleeter：{model?.ready ? '已就绪' : '未就绪'}</div>
            </div>
          ...
          <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            >
              {processing ? <Loader size={16} className="animate-spin" /> : <Split size={16} />}
              开始分离
            </button>
          </section>
        </aside>
        ...
        <main className="min-h-0 overflow-auto">
          ...
          {result ? (
            <section className="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
              <div className="border-b border-gray-200 p-4 dark:border-gray-700">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h2 className="text-sm font-semibold">分离完成</h2>
                    <p className="mt-1 text-xs text-gray-500">已生成真实人声与伴奏 WAV 文件</p>
                  </div>
                </div>
              </div>
              <div className="grid gap-4 p-4 md:grid-cols-2">
                {[
                  { title: '人声', path: result.vocalsPath, size: result.vocalsSize, icon: Music2 },
                  { title: '伴奏', path: result.accompanimentPath, size: result.accompanimentSize, icon: Split },
                ].map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-2 break-all text-xs text-gray-500">{item.path}</p>
                      <p className="mt-1 text-xs text-gray-500">{formatBytes(item.size)}</p>
                      <audio className="mt-3 w-full" controls src={convertFileSrc(item.path)} />
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => invoke('show_in_folder', { path: item.path })}
            ...
            <section className="flex min-h-[560px] flex-col items-center justify-center rounded-lg border border-gray-200 bg-white p-8 text-center shadow-sm dark:border-gray-700 dark:bg-gray-800">
              <Split size={56} className="text-gray-300 dark:text-gray-600" />
              <h2 className="mt-4 text-base font-semibold">本地人声 / 伴奏分离</h2>
              <p className="mt-2 max-w-md text-sm leading-6 text-gray-500 dark:text-gray-400">
                使用 Spleeter 2-stem 模型离线分离人声与伴奏，不上传任何音频。
              </p>
            </section>
          )}
        </main>
